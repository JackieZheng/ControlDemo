...
/**
 * @param <T> 泛型参数类型为{@link BaseActivity} || {@link TopBarActivity}
 * 这个泛型参数将指定使用哪个Activity作为Fragment的容器
 */
public abstract class BaseFragment<T extends BaseActivity> extends Fragment {
   ...
   */
  public static void startActivity(BaseFragment fragment, Class clazz, Bundle bundle,
      int requestCode) {
   ...
   * @param requestCode 请求吗
   */
  public static void startActivity(Activity fragment, Class clazz, Bundle bundle, int requestCode) {
    if (bundle == null) bundle = new Bundle();
    bundle.putString(BaseActivity.FRAGMENT_CLASS_NAME, clazz.getName());
   ...
   */
  protected static Type getGenericType(Class clazz) {
